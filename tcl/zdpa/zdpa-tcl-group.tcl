...
if { $argc != 7 } {
        puts stderr {7 argues : Poisson, Construction(cons_ = 0: truncated; =1: solion) k, r, p, num_nodes, idle_time}
        exit 1
}
 
set test_ [lindex $argv 0]
set cons_ [lindex $argv 1]
set k_ [lindex $argv 2]
set r_ [lindex $argv 3]
set p_ [lindex $argv 4]
set n_node_ [lindex $argv 5]
set t_idle_ [lindex $argv 6]
...
Mac/Sat/ZDPA set construction $cons_;
...
proc finish {} {
	set f [open record2.txt a]
set pracotl [new PracOfferedTrafficLoad] 
set pracload [$pracotl returnPracLoad]
set sbnumoneotl [new SBNumClassOne]
set sbnumone [$sbnumoneotl returnSBNumClassOne]
set sbnumtwootl [new SBNumClassTwo]
set sbnumtwo [$sbnumtwootl returnSBNumClassTwo]

puts $pracload
puts $f "Practical sent bits $pracload" 
puts $f "SBNumClassOne $sbnumone"
puts $f "SBNumClassTwo $sbnumtwo"
close $f
	global ns outfile 
	$ns flush-trace
	close $outfile
